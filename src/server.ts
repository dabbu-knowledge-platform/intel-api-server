...
async function main() {
	...
	const server = app.listen(process.env.PORT || 0, () => {
		...
		// Set the DABBU_INTEL_API_SERVER_PORT environment variable
		process.env.DABBU_INTEL_API_SERVER_PORT = (
			server.address()! as AddressInfo
		).port.toString()

		console.log(
			`Server running on ${process.env.DABBU_INTEL_API_SERVER_PORT}`,
		)
	})
}
